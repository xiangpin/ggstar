...
`geom_star` is similar to `geom_point` of `r CRANpkg("ggplot2")`. The main difference is `geom_star` uses `starshape` to map the `starshapes`. Another is `geom_star` uses `scale_starshape_manual` for manual scaling. The shapes will not be distorted on cartesian coordinates or polar coordinates.
